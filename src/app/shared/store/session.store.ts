...
export const SessionStore = signalStore(
  ...
  withMethods(
    (store, sessionService = inject(SessionService), transcriptionService = inject(TranscriptionService)) => ({
        ...
        pipe(
          switchMap(({ sessionId, attachmentId, language, minSpeakers, maxSpeakers }) =>
            transcriptionService
              .triggerTranscription(sessionId, attachmentId, { language, minSpeakers, maxSpeakers })
              .pipe(
                tap(() => {
                  const session = store.session();
                  if (!session) return;
